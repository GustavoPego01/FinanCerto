...
language plpgsql security definer set search_path='' as $$ declare msg public.whatsapp_messages; code public.whatsapp_link_codes; conn public.whatsapp_connections; begin
  ...
  update public.whatsapp_messages set intent='link_verified',user_id=conn.user_id,connection_id=conn.id,connection_version=conn.version where id=msg.id;
  return true;
end $$;
